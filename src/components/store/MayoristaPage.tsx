    ...
    <section style={{ marginBottom: 40 }}>
      {catName && (
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }}>
          ...
          <button onClick={() => downloadCatalogoPDF(catName, prods)}
            style={{ padding: "7px 14px", borderRadius: 8, background: "rgba(59,130,246,.1)", border: "1px solid rgba(59,130,246,.3)", color: "#3b82f6", fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: "inherit", whiteSpace: "nowrap" }}>
            Descargar catalogo
          </button>
        </div>
      )}
      
      {/* GRILLA VERTICAL DE 2 COLUMNAS */}
      <div style={{ 
        display: "grid", 
        ...
        {visible.map(p => {
          ...
          return (
            <div 
              ...
              style={{
                background: "#ffffff",
                border: `1px solid ${inCart ? "rgba(59,130,246,.4)" : "#e5e7eb"}`,
                borderRadius: 14,
                overflow: "hidden",
                display: "flex",
              ...
              onMouseLeave={e => e.currentTarget.style.borderColor = inCart ? "rgba(59,130,246,.4)" : "#e5e7eb"}
            >
              {/* BOTÓN FAVORITO */}
              <button 
                ...
                style={{
                  ...
                  justifyContent: "center", cursor: "pointer", boxShadow: "0 2px 8px rgba(0,0,0,.1)", fontSize: 15
                }}
              >
                {esFav ? "❤️" : "🤍"}
              </button>
              ...
              {/* IMAGEN Y ETIQUETAS */}
              <div style={{ position: "relative", height: 140, overflow: "hidden", background: "#f8f8f8", width: "100%" }}>
                ...
                {inCart && (
                  <div style={{ position: "absolute", top: 6, right: 42, zIndex: 5, background: "#3b82f6", color: "#ffffff", borderRadius: 20, padding: "2px 7px", fontSize: 11, fontWeight: 700 }}>
                    ×{inCart.qty}
                  </div>
                )}
              </div>
              ...
              <div style={{ padding: "10px 12px 12px", flex: 1, display: "flex", flexDirection: "column", gap: 6 }}>
                ...
                {p.description && (
                  <p style={{ fontSize: 10, color: "#666", lineHeight: 1.4, overflow: "hidden", display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical" }}>
                    {p.description}
                  </p>
                )}
                ...
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: 2 }} onClick={e => e.stopPropagation()}>
                  <div>
                    <p style={{ fontSize: 16, fontWeight: 800, color: "#3b82f6" }}>{fmt(Number(p.price_wholesale))}</p>
                    <p style={{ fontSize: 10, color: "#999", textDecoration: "line-through" }}>{fmt(Number(p.price_retail))}</p>
                  </div>
                </div>

                {/* STOCK */}
                <p style={{ fontSize: 10, fontWeight: 700, color: Number(p.stock_quantity) === 0 ? "#ef4444" : Number(p.stock_quantity) <= 3 ? "#ef4444" : Number(p.stock_quantity) <= 10 ? "#f59e0b" : "#10b981" }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 6 }} onClick={e => e.stopPropagation()}>
                  <button 
                    ...
                    style={{ width: 26, height: 26, borderRadius: "50%", border: "1px solid rgba(59,130,246,.3)", background: "rgba(59,130,246,.08)", color: "#3b82f6", fontSize: 13, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}
                  >−</button>
                  ...
                  <input 
                    ...
                    style={{ width: "100%", textAlign: "center", border: "1px solid rgba(59,130,246,.3)", borderRadius: 6, fontSize: 11, padding: "3px 0", fontFamily: "inherit", color: "#1a1a1a", outline: "none", background: "rgba(59,130,246,.04)" }} 
                  />
                  
                  <button 
                    onClick={() => addToCart(p)} 
                    disabled={Number(p.stock_quantity) === 0} 
                    style={{ width: 26, height: 26, borderRadius: "50%", border: "1px solid rgba(59,130,246,.3)", background: "rgba(59,130,246,.08)", color: "#3b82f6", fontSize: 13, cursor: Number(p.stock_quantity) === 0 ? "not-allowed" : "pointer", display: "flex", alignItems: "center", justifyContent: "center", opacity: Number(p.stock_quantity) === 0 ? 0.5 : 1, flexShrink: 0 }}
                  >+</button>
                </div>
      ...
      {(hasMore || hasLess) && (
        <div style={{ display: "flex", gap: 8, justifyContent: "center", marginTop: 16 }}>
          {hasMore && (
            <button 
              onClick={() => setVisibleCount(v => Math.min(v + 6, prods.length))}
          ...
          {hasLess && (
            <button 
              ...
              style={{ padding: "8px 20px", borderRadius: 8, background: "rgba(255,255,255,.05)", border: "1px solid #e5e7eb", color: "#666", fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: "inherit" }}
            >
              Ver menos
            </button>
          )}
    ...
    <div style={{minHeight:"100vh",background:"#ffffff",fontFamily:"'DM Sans',system-ui,sans-serif",display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
      ...
      <div style={{width:"100%",maxWidth:420}}>
        <div style={{textAlign:"center",marginBottom:32}}>
          ...
          <p style={{color:"#ef4444",fontSize:12,fontWeight:700,marginTop:6}}>COMPRA MINIMA $80.000</p>
        </div>
  ...
  <div style={{ minHeight: "100vh", background: "#ffffff", color: "#1a1a1a", fontFamily: "'DM Sans',system-ui,sans-serif" }}>
    <style>{`
      ...
      *{box-sizing:border-box;margin:0;padding:0}
      ...
      .modal{position:fixed;inset:0;z-index:200;display:flex;align-items:flex-end;justify-content:center;background:rgba(0,0,0,.6);backdrop-filter:blur(8px)}
      @media(min-width:640px){.modal{align-items:center;padding:20px}}
      .modal-box{background:#ffffff;border:1px solid #e5e7eb;border-radius:20px 20px 0 0;padding:28px;width:100%;max-width:480px;max-height:92vh;overflow-y:auto}
    ...
    {/* HEADER COMPACTO */}
<header style={{ position: "sticky", top: 0, zIndex: 50, borderBottom: "1px solid #e5e7eb", background: "#ffffff", padding: "10px 16px" }}>
  <div style={{ maxWidth: 1400, margin: "0 auto" }}>
    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 6 }}>
      <h1 style={{ fontFamily: "'Syne',sans-serif", fontSize: 16, fontWeight: 800, margin: 0 }}>
        <span style={{ color: "#00B4D8" }}>Concepción</span> Tecnologia <span style={{ fontSize: 9, padding: "2px 6px", background: "#eff6ff", border: "1px solid #bfdbfe", borderRadius: 4, color: "#3b82f6", verticalAlign: "middle" }}>MAYORISTA</span>
      </h1>
      {cartCount > 0 && (
        <button onClick={() => setCartOpen(true)} style={{ padding: "5px 10px", borderRadius: 6, background: "#0077b6", color: "#fff", border: "none", fontSize: 11, fontWeight: 700, cursor: "pointer" }}>
          🛒 {cartCount}
        </button>
      )}
    </div>
    <p style={{ fontSize: 10, color: "#666", marginBottom: 8 }}>{regForm.name} · <span style={{ color: "#ef4444", fontWeight: 700 }}>Mínima $80.000</span></p>
    <div style={{ display: "flex", gap: 6, overflowX: "auto", paddingBottom: 4, scrollbarWidth: "none" }}>
      <button onClick={() => setShowFavs(true)} style={{ padding: "4px 8px", borderRadius: 6, background: "#f9fafb", border: "1px solid #e5e7eb", fontSize: 10, fontWeight: 600, cursor: "pointer" }}>❤️ Favoritos</button>
      <button onClick={() => { setShowHistorial(true); loadHistorial(); }} style={{ padding: "4px 8px", borderRadius: 6, background: "#f9fafb", border: "1px solid #e5e7eb", fontSize: 10, fontWeight: 600, cursor: "pointer" }}>Mis Pedidos</button>
      <a href="/" style={{ padding: "4px 8px", borderRadius: 6, background: "#f9fafb", border: "1px solid #e5e7eb", fontSize: 10, fontWeight: 600, textDecoration: "none", color: "#444" }}>Minorista</a>
    </div>
  </div>
</header>
...
<main style={{ maxWidth: 1400, margin: "0 auto", padding: "12px 16px" }}>
  ...
  <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
    <input className="if3" placeholder="Buscar precio mayorista" value={search} onChange={e => setSearch(e.target.value)} style={{ padding: "8px 12px", fontSize: 12, height: 38 }} />
<select 
  value={sort} 
...
>
  ...
  {/* Categorías */}
  <div className="sx2" style={{ marginBottom: 16 }}>
    <button className={`pb2 ${!activeCat ? "active" : ""}`} onClick={() => setActiveCat(null)} style={{ padding: "5px 12px", fontSize: 12 }}>Todos</button>
    {categories.map(c => (
      <button key={c.id} className={`pb2 ${activeCat === c.slug ? "active" : ""}`} onClick={() => setActiveCat(activeCat === c.slug ? null : c.slug)} style={{ padding: "5px 12px", fontSize: 12 }}>{c.icon} {c.name}</button>
    ))}
  </div>
